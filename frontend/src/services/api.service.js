...
export const inventoryAPI = {
  ...
  reserveTickets: async (eventId, quantity) => {
    const response = await api.put(API_ENDPOINTS.RESERVE_TICKETS(eventId, quantity))
    return response.data
  }
}
...
export const waitingRoomAPI = {
  joinQueue: async () => {
    const response = await api.post(API_ENDPOINTS.JOIN_QUEUE)
    return response.data
  },
  
  getQueueStatus: async () => {
    const response = await api.get(API_ENDPOINTS.QUEUE_STATUS)
    return response.data
  },
  
  leaveQueue: async () => {
    const response = await api.post(API_ENDPOINTS.LEAVE_QUEUE)
    return response.data
  }
}
